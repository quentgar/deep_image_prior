...
def rotate_gconv_kernels(kernel, periodicity=2 * np.pi, diskMask=True):
  ...
  #
  kernels_planar_rotated = torch.sparse.mm(rotOp_matrix.type(torch.DoubleTensor), kernel_flat.type(torch.DoubleTensor))
  ...
  for orientation in range(orientations_nb):
      ...
      kernels_temp = torch.reshape(kernels_temp, [orientations_nb, channelsOUT, channelsIN, kernelSizeH, kernelSizeW])  # [Nx,Ny,Nin,Nout,Ntheta]
